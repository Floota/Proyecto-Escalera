...
func _on_exit_pressed():
	...
	print("Regresando al menu principal desde el menu de pausa...")
...
func slot_pressed(slot_number):
	...
	if existing_game_data.size() > 0:
		...
		if empty_slot:
			print("El slot vacío. Guardando una nueva partida.")
			sql_manager.save_game_state(game_data, slot_number)
		else:
			$ColorRect/OptionsSlot.visible = true
			
	else:
		print("El slot vacio. Guardando una nueva partida.")
		sql_manager.save_game_state(game_data, slot_number)
	GLOBAL.save_collect(slot_number)
